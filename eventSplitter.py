...
def numbered_splitter(
    start, duration, window, p, s, tag, vPath, char, number, event=False
):  ## **DONE**
    if event:
        name = (
            p
...
def data_prep(splits):
    ...
    splits_temp = (
        splits.fillna(False).reset_index(drop=False).set_index(annotation_label)
    )
    try:
    ...
    behaviorSplits = (
        splits.fillna(False)
        .drop(
            columns=["Satisfied", "Confused", "Bored", "focused", "idle", "distarcted"]
        )
        .reset_index(drop=False)
        .set_index(["on-task", "off-tsak"])
    )
    emotionSplits = (
        splits.fillna(False)
        .drop(columns=["focused", "idle", "distarcted", "on-task", "off-tsak"])
        .reset_index(drop=False)
        .set_index(["Satisfied", "Confused", "Bored"])
    )
    attentionSplits = (
        splits.fillna(False)
        .drop(columns=["Satisfied", "Confused", "Bored", "on-task", "off-tsak"])
        .reset_index(drop=False)
        .set_index(["focused", "idle", "distarcted"])
    )
    ...
    try:
        ...
        print(
            "THERE ARE NO ON-TASK TAGS. THIS IS UNUSUAL, but is not a cause for concern"
        )
    try:
        ...
        print(
            "THERE ARE NO OFF-TASK TAGS. THIS IS UNUSUAL, but is not a cause for concern"
        )
    try:
        ...
        print(
            "THERE ARE NO SATISFIED TAGS. THIS IS UNUSUAL, but is not a cause for concern"
        )
    try:
        ...
        print(
            "THERE ARE NO CONFUSED TAGS. THIS IS UNUSUAL, but is not a cause for concern."
        )
    try:
        ...
        print(
            "THERE ARE NO BORED TAGS. THIS IS UNUSUAL, but is not a cause for concern"
        )
    try:
        ...
        print(
            "THERE ARE NO FOCUSED TAGS. THIS IS UNUSUAL, but is not a cause for concern"
        )
    try:
    ...
    try:
        ...
        print(
            "THERE ARE NO DISTRACTED TAGS. THIS IS UNUSUAL, but is not a cause for concern"
        )
    return output
...
def splitting_categories(category_name, category_array, window, p, s, vPath, char):
    ...
    for index, row in category_array.iterrows():
        ...
        number += 1
        numbered_splitter(
            start, duration, window, p, s, category_name, vPath, char, number
        )
...
if __name__ == "__main__":
    ...
    print(dic["on-task"])
    # NOTE; DATA PREP will get all available data as data frames, mapped to their name.  Use these in splitting categories, with whatever name you like
